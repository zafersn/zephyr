			       ...
			       int16_t *value)
{
	...
	case CELLULAR_SIGNAL_RSSI:
		ret = hl78xx_send_cmd(data, signal_cmd_csq, hl78xx_chat_callback_handler,
				      allow_match, 2);
		break;

	case CELLULAR_SIGNAL_RSRP:
	case CELLULAR_SIGNAL_RSRQ:
		ret = hl78xx_send_cmd(data, signal_cmd_cesq, hl78xx_chat_callback_handler,
				      allow_match, 2);
		break;
...
static enum cellular_access_technology hl78xx_rat_to_access_tech(enum hl78xx_cell_rat_mode rat_mode)
{
	switch (rat_mode) {
					    ...
					    bool reset)
{
	...
	int r = hl78xx_send_cmd(data, cmd_string, NULL, &ok_match, 1);
					   ...
					   uint16_t matches_size)
{
	...
	int r = modem_dynamic_cmd_send(data, NULL, cmd, cmd_size, response_matches, matches_size,
				       true);
	return r;
}
