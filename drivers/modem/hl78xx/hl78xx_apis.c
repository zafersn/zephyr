...
/**
 * @brief Helper macro to extract GNSS context from either device type
 *
 * This reduces repetitive code in GNSS API functions.
 * Declares data_gnss and data_modem variables.
 * Works with either GNSS device or modem device.
 */
#define GNSS_GET_CONTEXT_OR_RETURN(dev, ret_val)                                                   \
...
{
	GNSS_GET_CONTEXT_OR_RETURN(dev, -EINVAL);
