...
#define HL78XX_TCP_STATUS_ID(x)                  ((x > 1 ? (x) - 1 : 0)) /* modem socket id is 1-based */
#define HL78XX_UDP_STATUS_ID(x)                  ((x > 1 ? (x) - 1 : 0)) /* modem socket id is 1-based */
...
/* Lightweight macro to emit a single-line debug log when needed. Prefer this
 * over many scattered `LOG_DBG("%d", __LINE__)` calls to reduce noise.
 */
#define HL78XX_DBG_LINE() LOG_DBG("%s:%d", __FILE__, __LINE__)
...
/**
 * @brief This function doesnt handle incoming UDP data.
...
void hl78xx_on_cgdcontrdp(struct modem_chat *chat, char **argv, uint16_t argc, void *user_data)
{
	...
	if (apn_field && strlen(apn_field) < sizeof(data->identity.apn)) {
...
static int handle_eof_pattern(struct hl78xx_socket_data *socket_data)
{
	...
	if (strncmp(&socket_data->receive_buf.buf[socket_data->parser_start_index_eof], EOF_PATTERN,
		...
		LOG_DBG("handle_eof_pattern: parser_socket_data_received=1 "
				      ...
				      struct modem_chat *chat)
{
	...
	if (socket_data->parser_state == HL78XX_PARSER_CONNECT_MATCHED &&
		...
		(void)handle_eof_pattern(socket_data);
	}
	parser_reset(socket_data);
	return true;
}
...
static void socket_process_bytes(struct hl78xx_socket_data *socket_data, char byte)
{
	...
	if (socket_data->parser_state != HL78XX_PARSER_ERROR_MATCHED &&
		...
		if (modem_chat_match_exact(socket_data, CONNECT_STRING)) {
			...
			/* CME error can be longer than the buffer length (prefix match). */
...
static int modem_process_handler(struct hl78xx_data *data)
{
	...
	LOG_DBG("modem_process_handler: post-process state=%d recv_len=%d recv_buf.len=%u "
	...
	/* Require EOF detection, OK confirmation, and that payload was pushed. */
...
void dns_work_cb(const struct device *dev, bool hard_reset)
{
	...
	if (hard_reset) {
		...
		if (!dnsCtx) {
			LOG_WRN("No default DNS resolver context available; skipping reconfigure");
				  ...
				  struct hl78xx_socket_data *socket_data)
{
	...
	if (ret < 0 ||
		...
		modem_socket_put(&socket_data->socket_config, sock->sock_fd);
		/* Map negative internal return codes to positive errno; fall back to EIO when
		 * the code is non-negative but the operation failed.
		 */
...
static int create_socket(struct modem_socket *sock, const struct sockaddr *addr,
			 struct hl78xx_socket_data *data)
{
	LOG_DBG("create_socket: entry fd=%d id=%d", sock->sock_fd, sock->id);
	int af;
	uint16_t dst_port;
	...
	LOG_DBG("create_socket: TCP/TLS socket, calling send_tcp_or_tls_config af=%d port=%u "
		"mode=%d",
		af, dst_port, mode);
	ret = send_tcp_or_tls_config(sock, dst_port, af, mode, data);
	LOG_DBG("create_socket: send_tcp_or_tls_config returned %d", ret);
	return ret;
}
			    ...
			    const size_t buf_len, int *sock_written)
{
	...
	while (len > 0) {
		LOG_DBG("send_data_buffer: waiting for TX semaphore (offset=%u len=%d)", offset,
			len);
		if (k_sem_take(&socket_data->mdata_global->script_stopped_sem_tx_int, K_FOREVER) <
		    0) {
			LOG_ERR("send_data_buffer: k_sem_take(tx) failed");
			return -1;
		}
			      ...
			      socklen_t optlen)
{
	...
	if (level == SOL_TLS) {
		int r = handle_tls_sockopts(obj, optname, optval, optlen);
		if (r < 0) {
			hl78xx_set_errno_from_code(r);
			return -1;
		}
		return 0;
	}
...
static ssize_t offload_sendmsg(void *obj, const struct msghdr *msg, int flags)
{
	...
	while (full_len > sent) {
		...
		if (ret < 0) {
			/* Map negative internal return code to positive errno; fall back to EIO */
			hl78xx_set_errno_from_code(ret);
			return -1;
		}
		sent += ret;
