...
<<<<<<< HEAD
	...
	if ((ret = k_sem_take(&socket_data.mdata_global->script_stopped_sem_tx_int, K_FOREVER)) <
		    0 ||
	    (ret = modem_pipe_transmit(socket_data.mdata_global->uart_pipe, sendbuf,
				       strlen(sendbuf))) < 0 ||
			      ...
			      const struct sockaddr *to, socklen_t tolen)
{
	...
	int ret = send_socket_data(obj, &msg, K_SECONDS(MDM_CMD_TIMEOUT));
	if (ret < 0) {
		errno = -ret;
		return -1;
	}
...
#define MODEM_DEVICE_SWIR_HL7812(inst) MODEM_HL78XX_DEFINE_INSTANCE(inst)
