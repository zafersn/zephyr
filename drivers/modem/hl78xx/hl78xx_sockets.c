				 ...
				 void *user_data)
{
	...
	if (is_ipv4) {
		...
		ret = net_addr_pton(AF_INET6, temp_addr_str, &new_ipv6_addr);
	}
#endif
	if (ret < 0) {
	...
	if (is_ipv4) {
		...
		ret = net_addr_pton(AF_INET6, socket_data.dns_v6_string, &dns_v6);
...
void iface_status_work_cb(struct hl78xx_data *data, modem_chat_script_callback script_user_callback)
{
	...
	ret = modem_cmd_send_int(data, script_user_callback, cmd, strlen(cmd), &cgdcontrdp_match,
				 1);
	if (ret < 0) {
				...
				size_t buf_len, k_timeout_t timeout)
{
	...
	ret = modem_cmd_send_int(socket_data.mdata_global, NULL, cmd_buf, strlen(cmd_buf),
				 connect_matches, ARRAY_SIZE(connect_matches));
	if (ret < 0) {
