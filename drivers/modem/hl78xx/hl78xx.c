...
static void hl78xx_enter_state(struct hl78xx_data *data, enum hl78xx_state state);
static void hl78xx_delegate_event(struct hl78xx_data *data, enum hl78xx_event evt);
...
int modem_cmd_send_int(struct hl78xx_data *user_data,
...
static int hl78xx_on_run_rat_cfg_script_state_enter(struct hl78xx_data *data)
{
	int ret = 0;
...
#endif
#endif
	if (rat_config_request != data->status.registration.rat_mode) {
...
static int hl78xx_on_enable_gprs_state_enter(struct hl78xx_data *data)
{
	...
	data->identity.apn[0] = '\0';
	strncat(data->identity.apn, CONFIG_MODEM_HL78XX_APN, sizeof(data->identity.apn) - 1);
...
static int hl78xx_on_state_enter(struct hl78xx_data *data)
{
	...
	case MODEM_HL78XX_STATE_RUN_RAT_CONFIG_SCRIPT:
		ret = hl78xx_on_run_rat_cfg_script_state_enter(data);
		break;
...
{
	int ret;
	const struct hl78xx_config *mdm_config = (const struct hl78xx_config *)dev->config;
	...
	const struct gpio_dt_spec *gpio_pins[GPIO_CONFIG_LEN] = {
		&mdm_config->mdm_gpio_reset,        &mdm_config->mdm_gpio_wake,
		&mdm_config->mdm_gpio_vgpio,
#if HAS_GPIO6_GPIO
		&mdm_config->mdm_gpio_gpio6,
#endif
#if HAS_PWR_ON_GPIO
		&mdm_config->mdm_gpio_pwr_on,
#endif
#if HAS_FAST_SHUTD_GPIO
		&mdm_config->mdm_gpio_fast_shtdown,
#endif
#if HAS_UART_DTR_GPIO
		&mdm_config->mdm_gpio_uart_dtr,
#endif
	};
...
#endif
#if HAS_GPIO6_GPIO
		{&mdm_config->mdm_gpio_gpio6, GPIO_OUTPUT, "GPIO6"},
#endif
	};
	...
	ret = gpio_add_callback(mdm_config->mdm_gpio_vgpio.port, &data->gpio_cbs.vgpio_cb);
	if (ret) {
